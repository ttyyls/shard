...
impl<T> Arg<T> {
	...
	fn try_mut<N: std::fmt::Display>(&mut self, name: N, value: T) {
		if self.set { error!("'{}' may only be used once", name); }
		self.value = value;
	}
}
...
impl Args {
	...
	fn handle_arg(&mut self, argument: &str, arguments: &mut std::vec::IntoIter<String>) {
		let args: Vec<String> = match argument.starts_with("--") {
			true  => vec![argument.into()],
			false => argument.chars().skip(1).map(|c| format!("-{c}")).collect(),
		};
		...
		for (i, arg) in args.into_iter().enumerate() {
			...
			match arg.as_str() {
				...
				"-f" | "--file" => {
					err_if_arg_end!();
					let file = arguments.next().unwrap_or_else(|| error!("{arg} expected FILE"));

					self.file.try_mut(arg, Box::leak(file.into_boxed_str()));
				},
				"-o" | "--output" => {
					err_if_arg_end!();
					let output = arguments.next().unwrap_or_else(|| error!("expected file"));
				...
				_ => error!("unrecognized argument {arg}"),
			}
		}
	}
